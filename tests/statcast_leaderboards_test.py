...
def test_park_factor_yearly_badinput():
    def _test():
        with pytest.raises(ValueError):
            sl.park_factor_yearly(season=1900)
        with pytest.raises(ValueError):
            sl.park_factor_yearly(season=2025, bat_side="B")
        with pytest.raises(ValueError):
            sl.park_factor_yearly(season=2025, conditions="Rainy")
        with pytest.raises(ValueError):
            sl.park_factor_yearly(season=2025, rolling_years=5)
...
def test_park_factor_yearly_season_rolling_years():
    def _test():
        df = sl.park_factor_yearly(season=2025, rolling_years=3)
        ...
        df = sl.park_factor_yearly(season=2025, rolling_years=1)
...
def test_park_factor_yearly_bat_side():
    def _test():
        df = sl.park_factor_yearly(season=2015, bat_side="L", rolling_years=3)
        assert df.shape == (30, 19)
        assert df.select(pl.col("Year").unique()).item() == "2013-2015"
        assert df.select(pl.col("Park Factor").max()).item() == 113
        assert df.select(pl.col("Team").n_unique()).item() == 30

        df = sl.park_factor_yearly(season=2015, bat_side="R", rolling_years=3)
        assert df.shape == (30, 19)
        assert df.select(pl.col("Year").unique()).item() == "2013-2015"
...
def test_park_factor_yearly_conditions():
    def _test():
        df = sl.park_factor_yearly(season=2019, conditions="Day", rolling_years=3)
        assert df.shape == (30, 19)
        assert df.select(pl.col("Year").unique()).item() == "2017-2019"
        assert df.select(pl.col("Park Factor").max()).item() == 113
        assert df.select(pl.col("Team").n_unique()).item() == 30

        df = sl.park_factor_yearly(
...
def test_park_factor_distance_badinputs():
    def _test():
        with pytest.raises(ValueError):
            sl.park_factor_distance(season=1900)
        with pytest.raises(ValueError):
            sl.park_factor_distance(season=8000)
...
def test_park_factor_distance():
    def _test():
        df = sl.park_factor_distance(season=2023)
